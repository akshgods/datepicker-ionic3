...
//     this.dataProvider.api_getStoreTiming.subscribe((data)=>{
//     console.log('getStoreInfo123: ',data);
//      let apiCall= localStorage.getItem('apiCall');
//       if(data && apiCall == 'true'){
//         this.storeInfo=data.store_info;
//         var zonetime = this.address.delivery_zone_time[0];
//         var pre_order_maxdays = this.storeInfo.store_preorder_max_days;
//         var pre_closed_min = (this.storeInfo.store_pre_closed_min)? this.storeInfo.store_pre_closed_min : 0;
//         var todaysdate = moment();
//         var closedate = moment().add(pre_order_maxdays,'day');
//         //ganesh
//         var store_pre_closed_min = parseInt(data['store_info'].store_pre_closed_min);
//          store_pre_closed_min=(store_pre_closed_min) ? store_pre_closed_min : 0;
//   var currdate = new Date();

//         var datetodayobj  = this.getselectday(data,currdate.getDay());
...
//         //ganesh
//         var dayname = todaysdate.format('ddd').toLowerCase();
//         var todayOpenTimeHrs = this.getHoursMinArray(`store_${dayname}_open_time`);
//         var todayCloseTimeHrs = this.getHoursMinArray(`store_${dayname}_close_time`);
//        console.log('todayOpenTimeHrs: ',todayOpenTimeHrs);
//        console.log('todayCloseTimeHrs: ',todayCloseTimeHrs);
//       //  console.log('todaysdate: ',todaysdate);
//       //  console.log('closedate: ',closedate);
       
//        try {
//        todaysdate.hours(todayCloseTimeHrs[0]).minutes(todayCloseTimeHrs[1]);
//         // if(moment().isAfter(todaysdate) || (this.storeInfo[`store_${dayname}_open_time`]=='00:00:00' && this.storeInfo[`store_${dayname}_close_time`]=='00:00:00')){
//         //   todaysdate.add(1,'day');
//         // }
//         if(temp_currdate.getTime() >mycurrdateclose.getTime()|| (this.storeInfo[`store_${dayname}_open_time`]=='00:00:00' && this.storeInfo[`store_${dayname}_close_time`]=='00:00:00')){
//           todaysdate.add(1,'day');
//         }
...
//         console.log('todaysdate: ',todaysdate,' closedate: ',closedate)
//         let instance = this.initDatePicker(todaysdate,closedate);
          
//           instance.unbind('change').change(()=>{
//             var date_picker_instance = mobiscroll.instance('getDate');
//             date_picker_instance = moment(date_picker_instance);
//             let dayname = date_picker_instance.format('ddd').toLowerCase();
...
//             var opentime = moment(date_picker_instance).set({'hours':todayOpenTimeHrs[0],'minutes':todayOpenTimeHrs[1]}).add('minutes',zonetime);
//             console.log("store openTime:",opentime);          
//             var closetime = moment(date_picker_instance).set({'hours':todayCloseTimeHrs[0],'minutes':todayCloseTimeHrs[1]}).subtract(pre_closed_min,'minutes');
//             console.log("store closeTime:",closetime);
//             var openclosetime;
//             var stepper = 15;
//             var diffmin;
//             var dateChange = true;
//             console.log('date_picker_instance: ',date_picker_instance);
//             console.log('moment format: ',(moment().format('YYYY-MM-DD')));
//             console.log('moment date_picker_instance format',(moment(date_picker_instance).format('YYYY-MM-DD')));
            
//             console.log('check for if: ',(moment(moment().format('YYYY-MM-DD')).isSame(moment(date_picker_instance).format('YYYY-MM-DD'),'day')));
//             if(moment(moment().format('YYYY-MM-DD')).isSame(moment(date_picker_instance).format('YYYY-MM-DD'),'day')){
//               openclosetime = this.openclosetime(dayname,opentime,closetime,pre_closed_min,zonetime);
//               console.log("openclosetime:",openclosetime);
//               console.log('opentime: ',opentime);
              
// console.log(`%c subject1 `, 'background: #222; color: #bada55');
//           try{
//             if(moment().isAfter(opentime) && moment().isBetween(moment(opentime),moment(closetime))){
//               console.log('if part');
// console.log(`%c subject2 `, 'background: #222; color: #bada55');
              
//               let diffmin = openclosetime.close.diff(moment(),'minutes');
              
//               if(diffmin<=15)
//                 stepper = (diffmin%5==0 && diffmin!=0) ? 5 : 1;
// console.log(`%c subject3 `, 'background: #222; color: #bada55');
  
//                 var newopen = opentime.set({'hours':moment().hours(),'minutes':moment().minutes()});
//                 console.log('%c newopen.minutes(): ',newopen.minutes(),'background: #222; color: #bada55');
//                 console.log(' newopen.minutes(): ',newopen.minutes());
//                 if(newopen.minutes()%15!=0){
// console.log(`%c subject4 `, 'background: #222; color: #bada55');
                  
//                   let remaindermin  = newopen.minutes() % 15;
//                   let estimatedtime = (newopen.minutes() - remaindermin) + 15;
//                   newopen.set('minute',estimatedtime).add(zonetime,'minutes');
//                   openclosetime = this.openclosetime(dayname,newopen,closetime,pre_closed_min,zonetime);
//                 }else{ //ganesh start
// console.log(`%c subject44 `, 'background: #222; color: #bada55');
// try{
//  var now = new Date();
//    var opendatetime = new Date(now.getFullYear(),now.getMonth(), now.getDate(), now.getHours(), now.getMinutes()+parseInt(zonetime));
//    var day = currdate.getDay();
//    var getdateobj = this.getselectday(data,day);
...
//     openclosetime.open._d=opendatetime;
//     // let timeinstance = this.initTimePicker(openclosetime.open,openclosetime.close,stepper,this.langcode,dateChange);
...
//             }else{
//               console.log('else mobiscrol_set_dateinstance');
//               try {
// console.log(`%c subject5 `, 'background: #222; color: #bada55');

//                 let mobiscrol_set_dateinstance = mobiscroll.instance('setDate',todaysdate.add(1,'day')).change();
//                 openclosetime = this.openclosetime(dayname,opentime,closetime,pre_closed_min,zonetime);
  
//               } catch (error) {
//                 console.log('error in mobiscrol_set_dateinstance: ',error);
                
//               }
//             }
//           }catch(error){
// console.log('date error new: ',error);

//           }
            
//             }else{
// console.log(`%c subject6 `, 'background: #222; color: #bada55');

//               console.log('date change');
//               openclosetime = this.openclosetime(dayname,opentime,closetime,pre_closed_min,zonetime);
//             }
       
//             console.log(`openclosetime.open: ${openclosetime.open},openclosetime.close: ${openclosetime.close}, stepper: ${stepper} , this.langcode: ${this.langcode}, dateChange: ${dateChange}`);
//             let timeinstance = this.initTimePicker(openclosetime.open,openclosetime.close,stepper,this.langcode,dateChange);
  
//              this.datetime = this.getTimeDate();
//             console.log('datetime1: ',this.datetime);
...
//           instance.change();        
//         } catch (error) {
//           console.log("dateerror preivious: ",error);
